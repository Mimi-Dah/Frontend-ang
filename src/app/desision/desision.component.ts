...
export class DesisionComponent {
  ...
  onPageChange(event: PageEvent): void {
    ...
    this.documentService.getAllNotes(page, pageSize).subscribe((data: any[]) => {
      this.onchange = data;
      this.dataSource.data = this.onchange;
  
      // Désinfecter les URLs
      this.onchange.forEach(onchange => {
        onchange.safeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`${url}${onchange.file}`);
      });
    });
  }

  ngOnInit(): void {
    this.documentService.getsoumission().subscribe((data: any) => {
      if (Array.isArray(data)) {
        this.decisions = data;
        this.dataSource.data = this.decisions; // Set the data for the Material table
